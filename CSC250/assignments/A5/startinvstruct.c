...
int inventoryWrite(char fname[], struct stock inventory[], int len){
...
int main(int argc, char *argv[]) {
	...
	do {
		...
		switch (choice) {
			...
			case 14: {
				char fname[999];
				centerText("\e[7m", "INVENTORY FILE SAVER", "\e[0m\n\n", ' ', 35);
				printf("Save file locashion> ");
				scanf("%s", fname);
				if(inventoryWrite(fname, inventory, len)) {
					printf("Successfully saved inventory file.\n");
				} else {
					printf("Err: File could not be saved\n");
				}
				pause();
			}
		}
